...
<section class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1> <i class="fas fa-money-bill-wave"></i> Contabilidad Total</h1>
      </div>
      <div class="col-sm-6">
	    ...
	    <div class="modal-body">
	      ...
	      <form method="post" action="{{route('contabilidad.index')}}" id="form-registrar"  >
              ...
              <div class="input-group">
                ...
                <input type="number" step="1" min="1" max='100' value="50" class="form-control" id="porcentaje" name="porcentaje">
              </div>
           </div>
	      	<div class="input-group">
...
$('#form-registrar').submit(function(e){
	...
	$.ajax(url,{
		...
		$.each(response,function(index,element){
			...
			totalCobrar += element.deuda;
			
			html += '<tr>';
			html += '<td>'+element.deuda+'</td>';
			html += '<td>'+element.banca+'</td>';
		...
		//totalCobrar = totalCobrar*($('#porcentaje').val())/100;

		$('#tbody-resultado').html(html);

		$('#porcentaje-text').text('total a cobrar : '+totalCobrar+' Bs');
...
function cargar(){
	...
	tabla = $('#tabla').DataTable({
		"ajax":"{{route('contabilidadTotal.index')}}",
	    serverSide :true,
	    "columns":[
	        {data:'dia'},
	        {data:'jugado'},
